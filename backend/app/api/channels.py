...
def populate_webhook_urls(channel: Channel, base_url: str):

    """Adds the calculated secure webhook URL to the channel object (transient)."""
    if channel.type == ChannelType.TELEGRAM:
        token = channel.config.get("token")
        if token:
            secret = telegram_service.get_webhook_secret(token, channel.workspace_id)
            channel.webhook_url = f"{base_url}/api/v1/webhooks/telegram/{channel.workspace_id}/{secret}"
        ...
        if token:
            secret = whatsapp_service.get_webhook_secret(token, channel.workspace_id)
            channel.webhook_url = f"{base_url}/api/v1/webhooks/whatsapp/{channel.workspace_id}/{secret}"
            channel.verify_token = secret # Also expose secret as verify token
    elif channel.type == ChannelType.DISCORD:

        token = channel.config.get("token") or channel.config.get("bot_token")
        if token:
            channel.webhook_url = f"{base_url}/api/v1/webhooks/discord/{token}"
    elif channel.type == ChannelType.SLACK:
        channel.webhook_url = f"{base_url}/api/v1/webhooks/slack/events"
    return channel
...
async def list_channels(
    ...
    for channel in channels:
        populate_webhook_urls(channel, settings.BASE_URL)
    return channels
...
async def create_channel(
    ...
    if channel.type == ChannelType.TELEGRAM:
        ...
        if token:
            settings = get_settings()
            await telegram_service.set_webhook(token, settings.BASE_URL, workspace_id)
...
async def update_channel(
    ...
    if channel.type == ChannelType.TELEGRAM:
        ...
        if token:
            settings = get_settings()
            await telegram_service.set_webhook(token, settings.BASE_URL, channel.workspace_id)
