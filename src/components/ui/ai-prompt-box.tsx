...
const Textarea = React.forwardRef<HTMLTextAreaElement, TextareaProps>(({ className, ...props }, ref) => (
  <textarea
    className={cn(
      "flex w-full rounded-md border-none bg-transparent px-3 py-1.5 text-sm text-foreground placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-0 disabled:cursor-not-allowed disabled:opacity-50 min-h-[36px] resize-none scrollbar-thin scrollbar-thumb-border scrollbar-track-transparent hover:scrollbar-thumb-muted-foreground/20",
      className
    )}
    ref={ref}
    rows={1}
      ...
      <TooltipProvider>
        ...
        >
          <div
            ref={ref}
            className={cn(
              "rounded-2xl border border-border bg-card p-1.5 shadow-lg transition-all duration-300",
...
export const PromptInputBox = React.forwardRef((props: PromptInputBoxProps, ref: React.Ref<HTMLDivElement>) => {
  const { onSend = () => {}, value: externalValue, onValueChange: onExternalValueChange, onKeyDown, isLoading = false, placeholder = "Type your message here...", className, disableVoice = false, disabled = false } = props;
  const [internalInput, setInternalInput] = React.useState("");
  const input = externalValue !== undefined ? externalValue : internalInput;
  const setInput = onExternalValueChange || setInternalInput;
