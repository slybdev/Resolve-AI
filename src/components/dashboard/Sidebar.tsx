  ...
  return (
    <div className={cn(
      ...
      <div className={cn("mb-6 w-full", isCollapsed && "flex flex-col items-center")}>
        ...
        {(canAccess('all-conversations') || canAccess('assigned-to-me') || canAccess('unassigned') || canAccess('urgent-sla')) && (
        <SidebarSection title="Inbox" icon={Inbox} isCollapsed={isCollapsed} onToggle={onToggle} defaultOpen={true}>
          {(canAccess('all-conversations') || (allowedPages.includes('tickets') && !allowedPages.includes('all-conversations'))) && (
            <NavItem 
              icon={Inbox} 
              label={allowedPages.includes('all-conversations') || !allowedPages.length ? "All Conversations" : "My Ticket Chats"} 
              active={currentView === 'all-conversations'} 
              onClick={() => onViewChange('all-conversations')} 
        ...
        {(canAccess('tickets') || canAccess('functional-teams') || canAccess('sla-breaches')) && (
        <SidebarSection title="Support" icon={Ticket} isCollapsed={isCollapsed} onToggle={onToggle} defaultOpen={true}>
          {canAccess('tickets') && <NavItem icon={Ticket} label="All Tickets" active={currentView === 'tickets'} onClick={() => onViewChange('tickets')} isCollapsed={isCollapsed} />}
          {canAccess('functional-teams') && <NavItem icon={Users} label="Functional Teams" active={currentView === 'functional-teams'} onClick={() => onViewChange('functional-teams')} isCollapsed={isCollapsed} />}
          {canAccess('sla-breaches') && <NavItem icon={AlertCircle} label="SLA Breaches" active={currentView === 'sla-breaches'} onClick={() => onViewChange('sla-breaches')} isCollapsed={isCollapsed} />}
        </SidebarSection>
        )}
        ...
        {(canAccess('analyze') || canAccess('ai-settings') || canAccess('train') || canAccess('help-center') || canAccess('test') || canAccess('deploy')) && (
        <SidebarSection title="AI Agent" icon={Bot} isCollapsed={isCollapsed} onToggle={onToggle}>
          ...
          {canAccess('ai-settings') && <NavItem icon={Sparkles} label="Persona & Rules" active={currentView === 'ai-settings'} onClick={() => onViewChange('ai-settings')} isCollapsed={isCollapsed} />}
          ...
          {canAccess('help-center') && <NavItem icon={BookOpen} label="Public Help Center" active={currentView === 'help-center'} onClick={() => onViewChange('help-center')} isCollapsed={isCollapsed} />}
          ...
          {canAccess('deploy') && <NavItem icon={Rocket} label="Deploy" active={currentView === 'deploy'} onClick={() => onViewChange('deploy')} isCollapsed={isCollapsed} />}
        </SidebarSection>
        )}

        {(canAccess('website-chat') || canAccess('email') || canAccess('whatsapp') || canAccess('instagram') || canAccess('facebook') || canAccess('telegram') || canAccess('slack') || canAccess('discord') || canAccess('voice-ai')) && (
        <SidebarSection title="Channels" icon={Share2} isCollapsed={isCollapsed} onToggle={onToggle}>
          ...
          {canAccess('voice-ai') && <NavItem icon={Mic} label="Voice AI" active={currentView === 'voice-ai'} onClick={() => onViewChange('voice-ai')} isCollapsed={isCollapsed} />}
        </SidebarSection>
        )}

        {(canAccess('ai-automations') || canAccess('macros') || canAccess('escalations') || canAccess('workflows')) && (
