...
interface Conversation {
  ...
  time: string;
  isAI: boolean;
  status: 'open' | 'closed' | 'resolved' | 'snoozed' | 'pending';
  assigned_to?: string;
...
export const AllConversations = ({ 
  ...
  useEffect(() => {
    if (initialConversationId) {
      ...
      setIsListOpen(false); // Focused mode: hide list when directed from a ticket
  ...
  const [isListOpen, setIsListOpen] = useState(!initialConversationId); // Hide list by default if deep-linked
  ...
  useEffect(() => {
    const loadUser = async () => {
      try {
        ...
        console.error("Failed to fetch user in AllConversations:", err);
      }
    };
    loadUser();
  }, []);
  ...
  const currentConversation = conversationsList.find(c => c.id === selectedId);
  ...
  const handleCloseConversation = async () => {
    ...
    try {
      setIsClosing(true);
      await api.conversations.update(selectedId, { status: 'closed' });
      toast(
        "Conversation Resolved", 
        "The conversation (and any open ticket) has been closed.", 
        "success"
      );
      // Refresh list
      fetchConversations();
      ...
      toast("Error", err.message || "Failed to close conversation", "error");
    } finally {
      setIsClosing(false);
    }
  };

  const handleMemberAssigned = (user: any) => {
  ...
  const fetchSuggestions = useCallback(async (id: string) => {
    try {
      ...
      const suggestions = await api.conversations.getSuggestedReplies(workspaceId, id);
  ...
  useEffect(() => {
    if (selectedId) {
      ...
      // fetchSuggestions(selectedId); (Disabled by user request)
      setInputText(""); // Reset input when switching
    }
  ...
  useEffect(() => {
    if (selectedId) {
      ...
      if (msgCount > prevMsgCountRef.current) {
        ...
        // Fetch new suggestions if customer message arrives
        if (!isAgentMessage) {
            // fetchSuggestions(selectedId); (Disabled by user request)
        }
      }
  ...
  useEffect(() => {
    fetchConversations();
    const interval = setInterval(fetchConversations, 5000); // Poll every 5s (fallback)
    return () => clearInterval(interval);
  }, [workspaceId]);

  // ── Real-Time WebSocket (instant updates from Telegram, Discord, etc.) ──
  useEffect(() => {
    if (!workspaceId) return;
        ...
        ws.onmessage = (event) => {
          try {
            ...
            if (data.type === 'message.new') {
              // Instant refresh — no waiting for poll
              fetchConversations();
  ...
  useEffect(() => {
    if (selectedId) {
      fetchMessages(selectedId);
      // Fetch metadata if not in current list
      const existing = conversationsList.find(c => c.id === selectedId);
      if (existing) {
        setActiveConversationMetadata(existing);
      } else {
  ...
  return (
    <div className="flex h-full w-full overflow-hidden bg-transparent relative gap-2">
        ...
        <div className="px-6 py-5 space-y-4">
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-bold text-foreground">
              {assignedOnly ? 'My Ticket Chats' : 'Inbox'}
            </h1>
          </div>
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar">
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-20 px-6 text-center opacity-50">
              <div className="w-12 h-12 bg-muted/20 rounded-full flex items-center justify-center mb-4 mx-auto">
                {searchTerm ? <Search className="w-6 h-6" /> : <Inbox className="w-6 h-6" />}
              </div>
              <p className="text-sm font-medium">
                {searchTerm 
                  ? `No results found for "${searchTerm}"` 
                  : assignedOnly 
                    ? "Your ticket queue is empty" 
                    : "Inbox is empty"}
              </p>
              {!searchTerm && (
                <p className="text-[10px] uppercase font-black tracking-widest mt-1">
                  {assignedOnly ? "Claim a ticket to start chatting" : "Total silence... for now"}
                </p>
              )}
            </div>
          ) : (
                        ...
                        <div className={cn(
                          ...
                          (chat.status === 'closed' || chat.status === 'resolved')
                            ...
                            : chat.assigned_to === currentUser?.id
                             ? (chat.priority === 'pending' ? "bg-amber-500/10 text-amber-600 border-amber-500/20" : "bg-primary/10 text-primary border-primary/20")
          ...
          <>
            ...
            <div className="h-16 border-b border-border flex items-center justify-between px-4 shrink-0 bg-card z-10">
              ...
              <div className="flex items-center gap-1.5 ml-4 shrink-0">
                {/* AI Copilot disabled by user request */}
                {/* 
                <button 
                  onClick={() => {
                  ...
                  <button 
                    onClick={async () => {
                      ...
                      try {
                        let ticket = null;
                        try {
                           ticket = await api.tickets.getByConversation(selectedId);
                        } catch (e) {}

                        if (ticket) {
                           await api.tickets.handoff(ticket.id);
                        } else {
                           await api.conversations.update(selectedId, { assigned_to: null });
                        }
                        
                        toast("Handed to AI", "The AI Assistant has resumed control of this chat.", "success");
                        
                        // Optimistic Update
                        setConversationsList(prev => prev.map(c => 
                          c.id === selectedId ? { ...c, routing_mode: 'ai', assigned_to: undefined } : c
                        ));
                    ...
                    className="flex items-center gap-2 px-3 py-1.5 bg-accent hover:bg-primary/10 text-muted-foreground hover:text-primary transition-all rounded-full group border border-border"
                    title="Hand off to AI"
                  >
                    <Bot className="w-3.5 h-3.5 transition-transform group-hover:rotate-12" />
                    <span className="hidden md:inline text-[11px] font-bold">Handoff</span>
                  </button>
                )}
                      ...
                      <>
                        {msg.sender === 'customer' ? (
                          ...
                          <div className={cn(
                            ...
                            {msg.sender === 'ai' ? (
                              <img src="/bot-avatar.png" className="w-full h-full object-cover rounded-full shadow-inner shadow-primary/10" alt="AI" />
            ...
            <div className="p-6 border-t border-border bg-card relative">
              ...
              {currentConversation?.assigned_to && currentConversation?.assigned_to !== currentUser?.id && !['closed', 'resolved'].includes(currentConversation?.status) ? (
                <div className="absolute inset-x-0 bottom-0 top-0 z-20 bg-background/80 backdrop-blur-sm flex flex-col items-center justify-center p-6 text-center border-t border-border">
                  ...
                  <h4 className="text-sm font-black text-foreground uppercase tracking-widest mb-1">Conversation Locked</h4>
                ...
                <div className="absolute inset-x-0 bottom-0 top-0 z-20 bg-primary/5 backdrop-blur-[2px] flex flex-col items-center justify-center p-6 text-center border-t border-primary/20">
                  ...
                  <h4 className="text-sm font-black text-foreground uppercase tracking-widest mb-1">AI Assistant Responding</h4>
                  <p className="text-[10px] text-muted-foreground font-bold uppercase tracking-tight max-w-[250px] mb-4">
                    The AI is currently handling this conversation. You must take over to send a manual message.
                  </p>
                  <button 
                    onClick={async () => {
                      if (!selectedId) return;
                      try {
                        ...
                        try {
                           ...
                           console.log("No ticket found for conversation, falling back to direct assignment");
                        }

                        if (ticket) {
                           await api.tickets.claim(ticket.id);
                           toast("Ownership Taken", "You have successfully taken control of the ticket.", "success");
                        } else {
                           await api.conversations.update(selectedId, { assigned_to: currentUser?.id });
                           toast("Take Over Success", "You have taken control of this conversation.", "success");
                        }

                        // Optimistic Update
                        setConversationsList(prev => prev.map(c => 
                          c.id === selectedId ? { ...c, routing_mode: 'human', assigned_to: currentUser?.id } : c
                        ));
                        
                        fetchConversations();
                      } catch (err: any) {
                        toast("Takeover Failed", err.message, "error");
                      }
                    }}
                    className="flex items-center gap-2 px-4 py-2 bg-primary text-primary-foreground text-[10px] font-black uppercase tracking-[0.2em] rounded-full shadow-lg shadow-primary/20 hover:scale-105 active:scale-95 transition-all"
                  >
              ...
              <div className="flex items-center justify-between gap-4 mb-4">
                ...
                {/* Second Handoff Button in Input Area */}
                {currentConversation?.routing_mode === 'human' && (
                  <button 
                    onClick={async () => {
                      if (!selectedId) return;
                      try {
                        ...
                        toast("Handed to AI", "The AI Assistant has resumed control.", "success");
                    ...
                    className="flex items-center gap-1.5 px-3 py-1 bg-primary/10 hover:bg-primary/20 text-primary rounded-lg text-[10px] font-black uppercase tracking-widest transition-all group"
                  >
                    <Bot className="w-3 h-3 transition-transform group-hover:rotate-12" />
                    Hand off to AI
                  </button>
                )}
              </div>
              <div className="relative">
                <AnimatePresence>
                  {showMacros && (
                    <motion.div 
                      ...
                      className="absolute bottom-[calc(100%+8px)] left-0 w-64 bg-card border border-border rounded-2xl shadow-2xl overflow-hidden z-50 text-left"
                    >
                      ...
                      <div className="max-h-56 overflow-y-auto p-2 space-y-1 no-scrollbar">
                        {macros.length > 0 ? macros.map(macro => (
                          <button 
                            ...
                            onClick={() => {
                              let body = macro.body;
                              const custName = activeConversationMetadata?.customerName || currentConversation?.customerName;
                              if (custName) {
                                body = body.replace(/\{\{customer\.name\}\}/gi, custName);
                              }
                          ...
                          <div className="py-4 text-center text-[10px] text-muted-foreground">No macros found. Create one in Macros & Snippets.</div>
                        )}
                      </div>
                    </motion.div>
                  )}
                ...
                <PromptInputBox 
                  ...
                  disabled={currentConversation?.assigned_to && currentConversation?.assigned_to !== currentUser?.id}
                ...
                isLoading={isLoading}
                placeholder={activeTab === 'reply' ? "Type a message or use / for macros..." : "Type an internal note (only visible to teammates)..."}
        ...
        {selectedId && (
          <>
            <div className="p-6 border-b border-border bg-accent/10 relative">
              ...
              <div className="grid grid-cols-2 gap-2">
                <button 
                  ...
                  className="flex items-center justify-center gap-2 py-2.5 bg-card hover:bg-accent border border-border rounded-xl text-[11px] font-bold text-foreground transition-all btn-press shadow-sm"
                >
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-8 no-scrollbar">
              ...
              {/* AI Suggested Replies disabled by user request */}
              {/* 
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                ...
                <div className="space-y-2">
                  {suggestedReplies.length === 0 && !isSuggesting ? (
                    <p className="text-[10px] text-muted-foreground italic px-2">No suggestions for this context.</p>
                  ) : (
                    suggestedReplies.map((reply, i) => (
                      <button 
                        key={i} 
                        onClick={() => setInputText(reply)}
                        className="w-full text-left p-3.5 bg-accent/30 hover:bg-accent border border-border rounded-xl text-[11px] text-foreground transition-all btn-press group relative overflow-hidden active:scale-[0.98]"
                      >
      ...
      <AssignMemberModal 
        ...
        teamId={currentConversation?.assigned_team_id}
        currentAssigneeId={currentConversation?.assigned_to}
      ...
      <CreateTicketModal 
        ...
        workspaceId={workspaceId}
        conversationId={selectedId || ""}
        customerName={conversationsList.find(c => c.id === selectedId)?.customerName || "Customer"}
      />
